...
func TestCoreDomainsDoNotAddAuthoritativeFloatFields(t *testing.T) {
	...
	specificExceptions := map[string]string{
		...
		"benchmark.AchievementEvidence.PortfolioReturnPct":         "achievement presentation evidence; upstream rule-engine input (EvaluationContext) is float64 end to end, see below",
		"benchmark.AchievementEvidence.BenchmarkReturnPct":         "achievement presentation evidence; upstream rule-engine input (EvaluationContext) is float64 end to end, see below",
