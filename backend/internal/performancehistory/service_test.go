...
func rankedAt(epoch, at time.Time, index float64, status performance.Status) *performance.RankedPerformance {
	return &performance.RankedPerformance{
		PortfolioID: "pf-1", RankedIndex: index,
		RankedReturnPercentage: index - 100, Status: status,
		TrackingStartedAt: epoch, ValuationAsOf: at,
		DataQualityStatus: "complete",
	}
}

func insertPoint(t *testing.T, repo Repository, id string, epoch, at time.Time, index float64, status performance.Status, kind SnapshotKind, quality QualityStatus) {
	t.Helper()
	s := Snapshot{
		ID: id, PortfolioID: "pf-1", UserID: "u1",
		TrackingStartedAt: epoch, RankedIndex: index, RankingStatus: status,
...
func TestRecordCurrentUsesPersistentRankedIndexAndIsBucketIdempotent(t *testing.T) {
	...
	for _, point := range points {
		assert.Equal(t, 112.3456789, point.RankedIndex)
...
func TestWindowCalculatesRankedReturnBoundariesAndCoverage(t *testing.T) {
	...
	for day := 0; day <= 30; day++ {
		...
		insertPoint(t, repo, "d-"+at.Format("20060102"), epoch, at, index, performance.StatusActive, KindDaily, QualityComplete)
	}
...
func TestWindowAllowsAfterBoundaryWithinToleranceAndRejectsOutside(t *testing.T) {
	...
	for day := 1; day < 30; day++ {
		...
		insertPoint(t, repo, "p-"+at.Format("2006010215"), epoch, at, 100+float64(day), performance.StatusActive, KindDaily, QualityComplete)
	}
...
func TestPausedCoveragePreventsGaming(t *testing.T) {
	...
	for day := 0; day <= 30; day++ {
		...
		insertPoint(t, repo, "p-"+at.Format("20060102"), epoch, at, 100+float64(day)/3, status, KindDaily, QualityComplete)
	}
...
func TestShortPauseWithinCoverageThresholdRemainsEligible(t *testing.T) {
	...
	for day := 0; day <= 30; day++ {
		...
		insertPoint(t, repo, "short-pause-"+at.Format("20060102"), epoch, at,
...
func TestTransitionSuppressionAndResumeBoundary(t *testing.T) {
	...
	input := performance.TransitionSnapshot{
		...
		RankedIndex: 100, Status: performance.StatusActive,
